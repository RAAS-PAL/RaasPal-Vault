...
|---|---|---|---|
| [[RaasPal-Internal-Ops-backend]] | `cf04b84` on `main` — PR #6 merged 2026-09-11 (case report + Solution AI + PM planner V39) | clean | ✅ **live on Lightsail behind `api.raaspal.com` since 2026-09-11 10:21 UTC** — Flyway applied V39 on that start (`38 → 39`), container `(healthy)`, `/v3/api-docs` 200 through nginx+TLS. Case-report endpoints not yet exercised with a login; `MONDAY_API_TOKEN`/`ANTHROPIC_API_KEY` presence in `deploy/api.env` unverified (see [[history]] 2026-09-11b) |
...
- [ ] **Confirm the case-report endpoints on the live box** with a real login: `GET /api/v1/case-reports/sync/status` (DB) and `GET /mk?refresh=true` (monday + Haiku). A Solution column of raw `dd-MMM` snippets means `ANTHROPIC_API_KEY` is missing from `deploy/api.env`; a monday error means `MONDAY_API_TOKEN` is.
