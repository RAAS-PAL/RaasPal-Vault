...
| [[robot-recommendation-web-raaspal]] | `4141628` on `main` (remote `RAAS-PAL/RaasPal-Ops-frontend`) — PR #4 merged the pending-case page, PR #6/#7 the PM planner; local `main` and `feat/dailycasereport-page` both fast-forwarded to it 2026-09-11 | clean | ✅ **live at `ops.raaspal.com`** (Vercel). Its `/api/v1/*` proxy route needs `BACKEND_PROXY_TARGET=https://api.raaspal.com` in the Vercel env — it was missing on 2026-09-11 and every login 502'd with "Proxy could not reach http://localhost:8080" until set |
